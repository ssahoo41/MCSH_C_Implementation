...
extern "C" int calculate_gmpordernorm(double** cell, double** cart, double** scale, int* pbc_bools,
                                        int* atom_i, int natoms, int* cal_atoms, int cal_num,
    ...
    //Allocating memory natoms x 4 array - each bin_i is 32 bytes to hold 4 8 byte integers
    ...
    //This is the cross product of the cell vectors in [1,2][:] dotted with the cell vector in [0][:] which is in fact the volume of the cell
    //The cross array is the cofactor matrix, the inv is the adjoint of the cofactor matrix divided by the determinant of cell matrix
    //Determinant of cell matrix is also the volume
    ...
    // I think this is getting the distance between planes - not sure how or which planes
    // I think it is the distance between x,y, and z planes of cell
    // Slicing space into volume elements based off of cutoff distance? This makes sense - really large cutoff distance, all atoms are effectively in the same bin
    // smaller cutoff distance, more bins - e.g. if cutoff distance is the plane distance, then there will be one bin (considered for 3 dimensions)
    for (int i=0; i<3; ++i) {
        tmp = 0;
        for (int j=0; j<3; ++j) {
    ...
    for (int i=0; i<natoms; ++i) {
        for (int j=0; j<3; ++j) {
            // multiplying scaled position along each axis (0,1) by the number of bins to identify the bin along each axis the atom will occupy
            // assigning double type variables into an int truncates the double. 
            bin_i[i][j] = scale[i][j] * (double) nbins[j];
        }
        // I think this line is basically getting a count of the number of atoms in each bin. 
        bin_i[i][3] = bin_i[i][0] + nbins[0]*bin_i[i][1] + nbins[0]*nbins[1]*bin_i[i][2];
        atoms_bin[bin_i[i][3]]++;
    }

    // Finds bin with the most atoms
    max_atoms_bin = 0;
    for (int i=0; i < total_bins; ++i) {
    ...
    neigh_check_bins = 1;
    for (int i=0; i < 3; ++i) {
        // nbins = ceil(plane_d/cutoff) < plane_d/cutoff +1 > plane_d/cutoff -> ceil(cutoff*nbins/plane_d) between 1 and 2 always - will usually be 2 except when 
        // plane_d/cutoff <=1
    ...
    for (int ii=0; ii < cal_num; ++ii) {
        int i=cal_atoms[ii];
        //printf("%f,%f,%f\n",cart[ii][0],cart[ii][1],cart[ii][2]);
        // calculate neighbor atoms
        // Don't get factor of 4 and 2
                ...
                if (mcsh_type == 1){
                    ...
                    for (int j = 0; j < nneigh; ++j) {
                        ...
                        for (int g = 0; g < ngaussians[neigh_atom_element_order]; ++g){
                            ...
                            //printf("%d\n",j);
                            //printf("%f,%f,%f,%f,%f,%f,%f\n",r0_sqr,x0,y0,z0,deriv[0],deriv[1],deriv[2]);
                        }
                        //if (ii < 1) printf("%f,%f,%f\n",sum_dmiu_dxj[j],sum_dmiu_dyj[j],sum_dmiu_dzj[j]);
                    }
                    sum_square += group_coefficient * sum_miu * sum_miu;
                    //printf("%f,%f\n",sum_miu,group_coefficient);
                ...
                if (mcsh_type == 2){
                    ...
                    for (int j = 0; j < nneigh; ++j) {
                        ...
                        for (int g = 0; g < ngaussians[neigh_atom_element_order]; ++g){
                            ...
                            //if (ii < 1) printf("%f,%f,%f,%f,%f,%f,%f\n",r0_sqr,x0,y0,z0,miu[0],miu[1],miu[2]);
                            //if (ii < 1) printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",r0_sqr,x0,y0,z0,deriv[0],deriv[1],deriv[2],deriv[3],deriv[4],deriv[5],deriv[6],deriv[7],deriv[8]);
                        }
                        //if (ii < 1) printf("%f,%f,%f\n",sum_miu1,sum_miu2,sum_miu3);
                        //if (ii < 1) printf("%f,%f,%f,%f,%f,%f,%f,%f,%f\n",sum_dmiu1_dxj[j],sum_dmiu1_dyj[j],sum_dmiu1_dzj[j],sum_dmiu2_dxj[j],sum_dmiu2_dyj[j],sum_dmiu2_dzj[j],sum_dmiu3_dxj[j],sum_dmiu3_dyj[j],sum_dmiu3_dzj[j]);
                    }
            ...
            for (int group_index = 1; group_index < (num_groups+1); ++group_index){
                ...
                if (mcsh_type == 1){
                    ...
                    for (int j = 0; j < nneigh; ++j) {
                        ...
                        int neigh_atom_element_index = nei_list_i[j*2];
                        std::cout << "index" << neigh_atom_element_index;
                        int neigh_atom_element_order = element_index_to_order[neigh_atom_element_index];
                        std::cout << "value: " << neigh_atom_element_order;
...
extern "C" int calculate_solid_gmpordernorm(double** cell, double** cart, double** scale, int* pbc_bools,
                                        int* atom_i, int natoms, int* cal_atoms, int cal_num,
                                        int** params_i, double** params_d, int nmcsh, double** atom_gaussian, int* ngaussians, int* element_index_to_order,
                                        double** mcsh, double** dmcsh) {
...
extern "C" int calculate_solid_gmpordernorm_noderiv(double** cell, double** cart, double** scale, int* pbc_bools,
                                        int* atom_i, int natoms, int* cal_atoms, int cal_num,
                                        int** params_i, double** params_d, int nmcsh, double** atom_gaussian, int* ngaussians, int* element_index_to_order,
                                        double** mcsh) {
