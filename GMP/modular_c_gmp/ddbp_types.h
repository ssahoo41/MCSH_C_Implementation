...
struct _DDBP_ELEM {
    ...
    double complex *v_tilde_cmplx; // basis functions (complex if not gamma-point) in the extended element
    double complex *v_cmplx;       // basis functions (complex if not gamma-point) in the element (restricted to E_k and orthognalized)
    double complex *v_prev_cmplx;  // basis functions from the previous SCF iteration
    int desc_v[9];
...
struct _DDBP_INFO {
    ...
    double complex *Hp_kpt; // subspace Hamiltonian (for k-points)
    double complex *Mp_kpt; // subspace mass matrix (for k-points)
    double complex *Q_kpt;  // eigenvectors (for k-points)
} DDBP_INFO;
