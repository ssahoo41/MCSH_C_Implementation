...
void build_nlist(const double rcut, const int nelem, const int natom, const double * const atompos,
	...
	for (i = 0; i < natom; i++){
		...
		yi = atompos[3*i+1];
		zi = atompos[3*i+2];
        //printf("atomtyp: %d xi: %f yi: %f zi: %f\n",natom_elem[atomtyp[i]],xi,yi,zi);
...
void initialize_gmpObj(GMPObj *gmp_str, NeighList *nlist, int *cal_atoms, int cal_num, int **params_i, double **params_d, int nmcsh, double** atom_gaussian, int* ngaussians, int* element_index_to_order){
    ...
    for (i=0; i < natom; i++){
        ...
        //printf("Neighbors for atom %d: %d\n",i,gmp_str->Nneighbors[i]);
    }
    gmp_str->unique_Nneighbors = (int *) malloc(sizeof(int)*natom);
    for (i=0; i < natom; i ++){
        gmp_str->unique_Nneighbors[i] = nlist->unique_Nneighbors[i];
        //printf("unique neighbors for atom %d: %d\n",i,gmp_str->unique_Nneighbors[i]);
    }
            ...
            for (int group_index = 1; group_index < (num_groups+1); ++group_index){
                ...
                if (mcsh_type == 1){
                    ...
                    for (int j = -1; j < nneigh[i]; ++j) {
						...
						//if (ii < 1) printf("%f,%f,%f\n",sum_dmiu_dxj[j+1],sum_dmiu_dyj[j+1],sum_dmiu_dzj[j+1]);
						//printf("\n");
                    }
                    sum_square += group_coefficient * sum_desc * sum_desc;
                    //printf("%f,%f\n",sum_desc,group_coefficient);
                ...
                if (mcsh_type == 2){
                    ...
                    for (int j = -1; j < nneigh[i]; ++j) {
						...
						//if (ii < 1) printf("%f,%f,%f,%f,%f,%f,%f,%f,%f\n",sum_dmiu1_dxj[j+1],sum_dmiu1_dyj[j+1],sum_dmiu1_dzj[j+1],sum_dmiu2_dxj[j+1],sum_dmiu2_dyj[j+1],sum_dmiu2_dzj[j+1],sum_dmiu3_dxj[j+1],sum_dmiu3_dyj[j+1],sum_dmiu3_dzj[j+1]);
                    }
